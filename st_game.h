...
{
    public:
        CGame(irr::IrrlichtDevice* device) : CState(device)
        {
        ...
        virtual bool OnEvent(const irr::SEvent& event)
        {
            if ( event.EventType == irr::EET_GUI_EVENT )
            {
                if (event.GUIEvent.EventType == irr::gui::EGET_BUTTON_CLICKED)
                {
                    ...
                    switch(id)
                    {
                        case GAME_IDBUTTON_MAINMENU:
                            se.UserEvent.UserData2 = game::STATE_MAINMENU; // событие - переключится на главное меню
